...
for act in active_list:
    ...
    if "签退" in name or "签退" in act.get("nameTwo", ""):
        print(f"签退活动: {json.dumps(act, ensure_ascii=False, indent=2)[:500]}")

# 打印所有activeType
print("\n所有activeType:")
types = set()
for act in active_list:
    at = act.get("activeType", -1)
    name = act.get("nameOne", "")
    types.add((at, name))
for at, name in sorted(types):
    print(f"  activeType={at}: {name}")
