...
(function ($) {
    ...
    var GesturePasswd= function (element, options) {
        ...
        this.triangleSize = options.triangleSize;
        this.triangleDistance = options.triangleDistance;
        this.triangleAngle = options.triangleAngle;
        this.siteX = 0;
        this.siteY = 0;
        ...
        this.pointDraw = function(c) {
            ...
            for (var p in that.sList) {
                if(p > 0) {
                    var endRadians = Math.atan((that.sList[p]["y"] - that.sList[p-1]["y"]) / (that.sList[p]["x"] - that.sList[p-1]["x"]));
                    endRadians += ((that.sList[p]["x"] >= that.sList[p-1]["x"]) ? 90 : -90) * Math.PI / 180;
                    that.drawArrowhead(that.sList[p-1]["x"], that.sList[p-1]["y"], endRadians);
                }
            }

            /*箭头方向时时转动，有待开发
             * if((that.sList.length - 1) > 0) {
            	var endRadians = Math.atan((that.siteY - that.sList[p]["y"]) / (that.siteX - that.sList[p]["x"]));
				endRadians += ((that.siteX >= that.sList[p]["x"]) ? 90 : -90) * Math.PI / 180;
				that.drawArrowhead(that.sList[p]["x"], that.sList[p]["y"], endRadians);
        	}*/
        };
        /**
         ...
         * 画箭头
         */
        this.drawArrowhead = function(x, y, radians) {
            /**
             * 根据弧度返回正切值
             */
            ...
            that.$ctx.moveTo(this.triangleSize/2, -this.triangleDistance);
            that.$ctx.lineTo(0, -(this.triangleDistance + getTanDeg(this.triangleAngle)*this.triangleSize));
            that.$ctx.lineTo(-this.triangleSize/2, -this.triangleDistance);
            that.$ctx.closePath();
            that.$ctx.restore();
            that.$ctx.fill();
        }
    ...
    GesturePasswd.DEFAULTS = {
        ...
        triangleSize: 14,  // 三角的大小
        triangleDistance: 10,  // 三角的距离
        triangleAngle: 45,    // 三角的夹角
    };
