...
```
用户A登录V2 API → 创建邀请(invite/create) → 生成inviteToken(30分钟有效)
                                                    ↓
用户B通过邀请链接 → 获取A的位置信息(invite/location) → 使用A的位置代签
                                                    ↓
                                    调用 sign/normal 或 sign/location 完成签到
```
...
## 漏洞2：CSRF - `/pptSign/updateSignStatusByUidsV2` (HIGH 7.5)
...
**2026-09-21 端到端实测确认**（新位置签到活动 aid=5000173244062，学生账号全程独立完成）：

```
[漏洞链复现步骤]
1. taskactivelist 获取进行中位置签到活动（按nameOne识别，activeType不可靠）
2. stuSignajax 距离泄露 — 提交3个远离城市坐标获取精确测距:
   北京(39.90,116.40) → "距教师指定签到地点626891.0米，不在可签到范围内。"
   上海(31.23,121.47) → "距教师指定签到地点831815.0米，不在可签到范围内。"
   广州(23.13,113.26) → "距教师指定签到地点1288574.0米，不在可签到范围内。"
3. 三边测量(trilateration)反推教师签到中心: (34.7208, 113.5812), RMS误差505m
4. 提交伪造坐标(34.7258, 113.5712) → 签到成功
5. 最终验证: /v2/apis/sign/signIn → status=1（已签到）

[签到记录佐证 — 伪造数据被服务端原样接受]
- latitude=34.7257616, longitude=113.571209   ← 伪造坐标入库
- useragent=伪造的手机UA                        ← 无设备校验
- clientip=发起请求的服务器IP                    ← 无IP地理一致性校验
- longitude_gd/latitude_gd=null                 ← 高德坐标未校验
